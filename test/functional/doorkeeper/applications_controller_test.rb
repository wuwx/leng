require 'test_helper'

class Doorkeeper::ApplicationsControllerTest < ActionController::TestCase
  setup do
    @application = FactoryGirl.create(:doorkeeper_application)
    @user = FactoryGirl.create(:user)
    @user.confirm!
    sign_in @user
  end

  test "should get index" do
    get :index
    assert_response :success
    assert_not_nil assigns(:applications)
  end

  test "should get new" do
    get :new
    assert_response :success
  end

  test "should create application" do
    assert_difference('Doorkeeper::Application.count') do
      post :create, application: { name: "Test1", redirect_uri: "http://www.test.com" }
    end

    assert_redirected_to oauth_application_path(assigns(:application))
  end

  test "should show application" do
    get :show, id: @application
    assert_response :success
  end

  test "should get edit" do
    get :edit, id: @application
    assert_response :success
  end

  test "should update application" do
    put :update, id: @application, application: {  }
    assert_redirected_to oauth_application_path(assigns(:application))
  end

  test "should destroy application" do
    assert_difference('Doorkeeper::Application.count', -1) do
      delete :destroy, id: @application
    end

    assert_redirected_to oauth_applications_path
  end
end
